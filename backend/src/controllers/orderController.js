...
const User = require('../models/User'); // 🆕 ADICIONADO para vinculação automática
const satisfactionTrigger = require('../services/satisfactionTriggerService');

// 🆕 FUNÇÃO AUXILIAR - Criar/Atualizar cliente automaticamente
const createOrUpdateCustomer = async (customerData) => {
  try {
    const { customerName, customerPhone, customerAddress } = customerData;
    
    if (!customerPhone) return null;
    
    // Buscar cliente existente pelo telefone
    let customer = await User.findOne({ telefone: customerPhone });
    ...
    if (customer) {
      ...
      if (customerAddress && customer.endereco !== customerAddress) {
        customer.endereco = customerAddress;
        updated = true;
      }
      
      if (updated) {
        await customer.save();
        console.log(`🔄 Cliente atualizado: ${customer.name} - ${customerPhone}`);
      }
      ...
      const newCustomer = new User({
        ...
        email: `${customerPhone}@cliente.temp`,
        password: 'temp123',
        role: 'cliente',
      ...
      console.log(`🆕 Novo cliente criado: ${newCustomer.name} - ${customerPhone}`);
      return newCustomer;
    }
  } catch (error) {
...
// 🆕 FUNÇÃO AUXILIAR - Atualizar métricas do cliente
const updateCustomerMetrics = async (customerId, orderValue) => {
  try {
    const customer = await User.findById(customerId);
    if (!customer) return;
    
    customer.loyalty.purchaseCount += 1;
    customer.loyalty.totalPurchases += orderValue;
    
    if (customer.loyalty.totalPurchases >= 500) {
      customer.loyalty.level = 'Platinum';
    } else if (customer.loyalty.totalPurchases >= 300) {
      customer.loyalty.level = 'Gold';
    } else if (customer.loyalty.totalPurchases >= 150) {
    ...
    await customer.save();
    console.log(`📈 Métricas atualizadas: ${customer.name} - ${customer.loyalty.level} - R$ ${customer.loyalty.totalPurchases}`);
  } catch (error) {
    console.error('Erro ao atualizar métricas do cliente:', error);
  }
};

// ✅ FUNÇÃO MODIFICADA - Criar novo pedido COM VINCULAÇÃO AUTOMÁTICA
exports.createOrder = async (req, res) => {
  try {
    const { items, total, customerName, customerPhone, customerAddress, paymentMethod, source, observations } = req.body;
    
    // 🆕 VINCULAÇÃO AUTOMÁTICA DE CLIENTE (apenas se houver telefone)
    let customer = null;
    if (customerPhone) {
      customer = await createOrUpdateCustomer({ customerName, customerPhone, customerAddress });
    }
    ...
    const orderData = {
      ...
      customerAddress,
      paymentMethod,
      source: source || 'manual' // 🆕 CAMPO OPCIONAL para rastrear origem
    };
    
    // 🆕 VINCULAR AO CLIENTE se foi criado/encontrado
    if (customer) {
    ...
    if (customer && total) {
      await updateCustomerMetrics(customer._id, parseFloat(total));
    }
    ...
    // ✅ RESPOSTA ORIGINAL PRESERVADA (exatamente como estava)
    res.status(201).json({ 
      success: true, 
...
exports.getOrdersByPhone = async (req, res) => {
  try {
    ...
    const orders = await Order.find({ customerPhone: phone })
      .sort({ createdAt: -1 })
      .select('_id customerName customerPhone total status createdAt items source observations');
